...
from denoisers.quadratic_spline import Quadratic_Spline_Func


def monotonic_clipping(cs, min_slope=0, max_slope=1.0):
    """Simple projection of the spline coefficients to obtain a monotonic linear spline"""
    device = cs.device
    n = cs.shape[1]
    # get the projected slopes
    new_slopes = torch.clamp(cs[:, 1:] - cs[:, :-1],
                             min=min_slope, max=max_slope)
    # clamping extension
    new_slopes[:, 0] = 0
    new_slopes[:, -1] = 0
...
def initialize_coeffs(init, grid_tensor, grid):
    ...
    if init == 'identity':
        ...
        raise ValueError(
            'init should be in [identity, relu, absolute_value, maxmin, max_tv].')
...
class LinearSplineDerivative_Func(torch.autograd.Function):
    ...
    def forward(ctx, x, coefficients_vect, grid, zero_knot_indexes, size, even):
        ...
        # (through linear interpolation) for each input in the B-spline range.
        activation_output = (
            coefficients_vect[indexes + 1] - coefficients_vect[indexes]) / grid
        if even:
            activation_output = activation_output + grid / 2
    ...
    def backward(ctx, grad_out):
        ...
        # grad_coefficients_vect.scatter_add_(0,
        #                                    indexes.view(-1) + 1,
        #          core.activations.                          (fracs * grad_out).view(-1))
        # left coefficients gradients
        # grad_coefficients_vect.scatter_add_(0, indexes.view(-1),
        #                                    ((1 - fracs) * grad_out).view(-1))
...
class LinearSpline(ABC, nn.Module):
    ...
    def __init__(self, mode, num_activations, size, range_, init="zero", monotonic_constraint=True, **kwargs):
        ...
        self.slope_max_attr = kwargs.get('slope_max', 1.0)
        self.slope_min_attr = kwargs.get('slope_min', 0.0)

        self.integrated_coeff = None

        # tensor with locations of spline coefficients
        self.grid_tensor = torch.linspace(-self.range_, self.range_,
                                          self.size).expand((self.num_activations, self.size))
        coefficients = initialize_coeffs(
            init, self.grid_tensor, self.grid)  # spline coefficients
    ...
    def monotonic_coefficients(self):
        ...
        return monotonic_clipping(self.coefficients, self.slope_min_attr, self.slope_max_attr)
    ...
    def derivative(self, x):
        ...
        assert x.size(1) == self.num_activations, \
            'Wrong shape of input: {} != {}.'.format(
                input.size(1), self.num_activations)
    ...
    def update_integrated_coeff(self):
        ...
        self.integrated_coeff = (
            self.integrated_coeff - self.integrated_coeff[:, (self.size + 2)//2].view(-1, 1)).view(-1)
    ...
    def integrate(self, x):
        ...
        x = Quadratic_Spline_Func.apply(x - self.grid.to(x.device), self.integrated_coeff, self.grid.to(
            x.device), self.zero_knot_indexes_integrated.to(x.device), (self.size + 2))
    ...
    def slope_max(self):
        coeff = self.projected_coefficients
        slope = (coeff[:, 1:] - coeff[:, :-1])/self.grid
