...
def prox_solution_L2(x, FB, FBC, F2B, FBFy, stepsize, sf):
    ...
    if not torch.is_tensor(stepsize):
        stepsize = torch.tensor([stepsize], device=x.device, dtype=x.dtype)

    if stepsize.ndim == 0:
        # Scalar tensor - expand to (b,1,1,1)
        stepsize = stepsize.view(1, 1, 1, 1).expand(b, 1, 1, 1)
    elif stepsize.ndim == 1:
        # 1D tensor (B,) - reshape to (B,1,1,1)
        if stepsize.numel() == 1:
            stepsize = stepsize.view(1, 1, 1, 1).expand(b, 1, 1, 1)
        else:
            stepsize = stepsize.view(-1, 1, 1, 1)  # (B,1,1,1)
    elif stepsize.ndim == 4:
        # assume already (b,1,1,1) or (b,c,1,1) etc.; keep as-is if broadcastable
        pass
    else:
        raise ValueError(
            "stepsize must have shape (b,) or (b,1,1,1) (or broadcastable to x).")
        
    # print(stepsize.shape)
    ...
    # If invW has batch=1 (shared kernel), expand it to batch B
    if invW.shape[0] == 1 and b > 1:
        invW = invW.expand(b, *invW.shape[1:])   # (B, C, H, W, 2)

    # Now make alpha broadcast to (B, C, H, W)
    alpha_real = alpha.expand(
        b, invW.shape[1], invW.shape[2], invW.shape[3])  # (B,C,H,W)
    
    invWBR = cdiv(FBR, csum(invW, alpha_real))
...
def Wiener_filter(x, k, stepsize, sf):
    ...
    stepsize = stepsize.to(device=x.device, dtype=x.dtype)

    if stepsize.ndim == 1:
        stepsize = stepsize.view(b, 1, 1, 1)          # (b,1,1,1)
    elif stepsize.ndim == 4:
        # assume already (b,1,1,1) or (b,c,1,1) etc.; keep as-is if broadcastable
        pass
    else:
...
def get_op_norm(fwd_op_kwargs, adjoint_op_kwargs, channels, device, img_size, n_iter=1000):
    x = torch.rand((1, channels, *img_size)).to(device).requires_grad_(False)

    with torch.no_grad():
        for i in range(n_iter):
            x = x / x.norm()
            x = Gt((G(x, **fwd_op_kwargs)), **adjoint_op_kwargs)
