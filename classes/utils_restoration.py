...
def complex_gaussian_noise_like(tensor, noise_level):
    # underlying real dtype (float32 if complex64, float64 if complex128)
    real_dtype = torch.float32 if tensor.dtype == torch.complex64 else torch.float64

    # generate real & imag noise
    real = torch.randn_like(tensor, dtype=real_dtype)
    imag = torch.randn_like(tensor, dtype=real_dtype)

    # build complex noise
    noise = torch.complex(real, imag) * (noise_level / math.sqrt(2))
